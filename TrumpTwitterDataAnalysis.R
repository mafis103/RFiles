...
trump_tweets <- map(2009:2020, ~sprintf(url, .x)) %>%
...
#so for the analysis here we will focus on what was tweeted between the day Trump 
#announced his campaign and election day. So we define the following table:
...
#coast (EST), it was tweeted then compute the proportion of tweets tweeted at each hour for 
#each device.
ds_theme_set()
campaign_tweets %>%
...
#Now we will study how their tweets differ. To do this we introduce the tidytext package.
...
#The most common tokens will be words, but they can also be single characters, ngrams, 
#sentences, lines or a pattern defined by a regex. The functions will take a vector of 
...
#English. For this reason, instead of using the default token, words, 
#we define a regex that captures twitter character. The pattern appears complex but 
#all we are defining is a patter that starts with @, # or neither and is followed by any 
...
#characteristics in our tokens. First, some of our tokens are just numbers (years for example). We want to remove these and we can find them using the regex ^\d+$. Second, some of our tokens come from a quote and they start with '. We want to remove the ' when it's at the start of a word, so we will use str_replace(). We add these two lines to the code above to generate our final table:
  tweet_words <- campaign_tweets %>% 
    mutate(text = str_replace_all(text, "https://t.co/[A-Za-z\\d]+|&amp;", ""))  %>%
    unnest_tokens(word, text, token = "regex", pattern = pattern) %>%
    filter(!word %in% stop_words$word &
...
#device was used to compose the tweet they came from, we can start exploring which words 
#are more common when comparing Android to iPhone.
...
#quantifying these differences. For each device and a given word, let's call it y, we 
#compute the odds or the ratio between the proportion of words that are y and not y and 
#compute the ratio of those odds. Here we will have many proportions that are 0 so we 
#use the 0.5 correction.
...
#We already see somewhat of a pattern in the types of words that are being tweeted 
#more in one device versus the other. However, we are not interested in specific words 
#but rather in the tone. Vaziri's assertion is that the Android tweets are more hyperbolic. 
#So how can we check this with data? Hyperbolic is a hard sentiment to extract from words as 
#it relies on interpreting phrases. However, words can be associated to more basic sentiment 
#such as as anger, fear, joy and surprise. In the next section we demonstrate basic sentiment 
#analysis.
  
##########Sentiment Analysis
#In sentiment analysis we assign a word to one or more "sentiment". 
#Although this approach will miss context dependent sentiments, such as sarcasm, 
...
#For example, the bing lexicon divides words into positive and negative. We can see 
#this using the tidytext function get_sentiments():
get_sentiments("bing")
  

#The AFINN lexicon assigns a score between -5 and 5, with -5 the most negative and 5 the 
#most positive.
...
#However, this somewhat complex since each tweet will have several sentiments attached to 
#it, one for each word appearing in the lexicon. For illustrative purposes, we will perform a 
#much simpler analysis: we will count and compare the frequencies of each sentiment appears 
#for each device.
...
#proportion of words with sentiment versus proportion of words without and then compute 
...
#So we do see some difference and the order is interesting: the largest three sentiments are 
#disgust, anger, and negative! But are they statistically significant? How does this compare 
#if we are just assigning sentiments at random?
...
#A graphical visualization shows some sentiments that are clearly overrepresented:
 ...
 #the Android in a way that is hard to explain by chance alone. Words not associated to a 
 #sentiment were strongly associated with the iPhone source, which is in agreement with the 
